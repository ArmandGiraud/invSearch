...
class TermsSearch():
    ...
    def build_answers(self):
        ...
        self.res = self.session.run(response_embeddings, {self.r_placeholder: [self.remove_stops(k) for k,v in self.suggest_dict.items()],
                                                    self.c_placeholder: ["" for k,v in self.suggest_dict.items()]})
